...
$("#load-map-input").on("change", (el)=>{
    ...
    reader.onload = async (e) => {
        ...
        document.getElementById("maptype").value = jsonData.type;
        document.getElementById("maptype").setAttribute("disabled", "true");
        mapType = jsonData.type;
        scene = await (new THREE.ObjectLoader()).parseAsync(jsonData.map);
        console.log(scene);
        ...
        gridHelper.position.x = 0;
        gridScene.add(gridHelper);
        jsonData.navmesh.filter(n=>n.obstacle).forEach(a=>{
        ...
        var spawnNode = jsonData.navmesh.find(n=>n.spawnNode);
        ...
        if(jsonData.type == "campaign"){
            var goalNode = jsonData.navmesh.find(n=>n.goalNode);
        ...
        new URLSearchParams(window.location.search).set("type", jsonData.type)
...
function saveMap() {
    ...
    download(    JSON.stringify({
        ...
        type: document.getElementById("maptype").value,
        navmesh: navMesh.nodes.map(n=>n.toSendable()),
    }) ,document.getElementById("name").value+'.json', 'application/json');
