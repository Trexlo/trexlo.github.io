...
class Player {
    constructor(id, nickname, color, image, ammo, mag, health, points, reloading, upgradeCost, attributes, position, rotation, dead) {
        ...
        $(tmp).find(".player-image").css('filter', 'hue-rotate('+((hsl.h*360)-120)+'deg)'); 
...
$('#hue-slider').on('input', (el)=>{
    $('#user-image').css('filter', `hue-rotate(${Number.parseInt(el.target.value)}deg)`);
})

$('#hue-slider')[0].value=(player && player.hueShift)?player.hueShift:Math.floor(Math.random()*360);
$('#player-name-input')[0].value=(player && player.name)?player.name:"Player";
$('#user-image').css('filter', `hue-rotate(${Number.parseInt($('#hue-slider')[0].value)}deg)`);
$('#create-button').on('click',()=>{
    $("#info-text").text('Checking if server is online, waking it up and connecting');
    console.log("creating");
    console.log($('#server-info-input')[0].value);
    var socketUnsafe = io("ws://" + $('#server-info-input')[0].value);
    var socketSafe = io("wss://" + $('#server-info-input')[0].value);
    socketSafe.emit("createLobby", (msg) => {
        if (msg.error) {
            console.log("Could not create: " + msg.message);
            ...
            window.location.assign('?server='+'wss://' + $('#server-info-input')[0].value+"&id="+msg.lobbyId);
        }
    });
    socketUnsafe.emit("createLobby", (msg) => {
        if (msg.error) {
            console.log("Could not create: " + msg.message);
            $(statusText).css("display", "block");
            $(continueButton).css("display", "block"); 
            ...
            socketUnsafe.disconnect();
            window.location.assign('?server='+'ws://' + $('#server-info-input')[0].value+"&id="+msg.lobbyId);
        }
    });
...
$('#start-button').on('click',()=>{
    ...
    socket.emit('startGame',{map:$('#map-select').val()});
})
$('#join-button').on('click',()=>{
    console.log("joining");
    if(urlParams.get('id') && urlParams.get('server')){
        if(!player)player={};
        player.name = $('#player-name-input')[0].value || "Guest";
        socket = io(urlParams.get('server'), {
        ...
        $('#url-span').text("Click here to copy");
        $('#url-span').on('click', ()=>{
            navigator.clipboard.writeText(window.location.href);
            $('#url-span').text("Copied!");
        });
        
        socket.on('lobbyChanged',(data) => {
            console.log("lobbyChanged");
            console.log(data);
            $("#player-list").children().remove();
            $('#start-button').css('display','none');
            $('#map-select').css('display','none');
            for(let p of data.players){
                let img = $('<img></img>');
                img.attr('src','./images/userImages/guest_green.png').css('filter',`hue-rotate(${p.hueShift}deg)`);
                let name = $('<p></p>');
                if(p.id == player.id && p.isOwner) {
                    $('#start-button').css('display','block');  
                    $('#map-select').css('display','block');
                    for(let m of data.maps){
                        let opt = $('<option></option>');
                        opt.text(m.mapName);
                        opt.attr('value', m.fileName);
                        $('#map-select').append(opt);
                    }
                }

                name.text(p.name+((p.isOwner)?" (owner)":""));
                let div = $('<div></div>');
                div.append(img);
                div.append(name);
                div.addClass('player-list-item');
                $("#player-list").append(div);
            }
        })
        socket.emit("joinLobby", {player: player, hueShift: Number.parseInt($('#hue-slider')[0].value) }, (msg) => {
            console.log(msg);
            socket.emit("test");
            if (msg.error) {
                ...
                $('#pre-join-menu').css('display','none')
                $('#lobby-menu').css('display','block')
                if(msg.started){
...
function checkState(){
    ...
    if(urlParams.get('id') && urlParams.get('server')){
        $('#create-game-menu').css('display','none')
        $('#pre-join-menu').css('display','flex')
    }
}
checkState();
function initSocket() {
...
async function load() {
    ...
    var model = await loader.loadAsync( './models/enemy.glb');
    var sizeVec =  new THREE.Vector3();
    ...
    var pmodel = await loader.loadAsync( './models/player.glb');
